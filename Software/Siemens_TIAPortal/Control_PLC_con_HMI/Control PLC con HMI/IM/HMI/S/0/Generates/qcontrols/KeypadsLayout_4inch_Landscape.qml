...
//                                                }
    ...
    onYChanged: {
        ...
        function changeSpecialCharacters(startindex)
        {
            for(var secondrowindex = 0; secondrowindex < row2.children.length; secondrowindex++)
            {
                row2.children[secondrowindex].value = Keyboard.numbers[startindex + secondrowindex];
            }

            for(var thirdrowindex = 0; thirdrowindex < row3.children.length-1; thirdrowindex++)
            {
                row3.children[thirdrowindex+1].value = Keyboard.numbers[startindex + thirdrowindex + Keyboard.row2Length];
            }
            row3.children[8].text = ( row3.children[0].text === specialkeysymbol1) ? Keyboard.numbers[26] :Keyboard.numbers[43];
        }
        ...
        Item
        {
            ...
            Rectangle
            {
                ...
                MouseArea
                {
                    ...
                    drag.minimumX: qm_KeyPadInlandscapeMode?0:-qm_DiffRotateX
                    drag.minimumY: qm_KeyPadInlandscapeMode?0:-qm_DiffRotateY
                    drag.maximumY: qm_KeyPadInlandscapeMode?qm_screenHeight-qVirtualKeyboardHeight:(qm_screenHeight-qVirtualKeyboardWidth)+qm_DiffRotateX
                    drag.maximumX: qm_KeyPadInlandscapeMode?qm_screenWidth-qVirtualKeyboardWidth:(qm_screenWidth-qVirtualKeyboardHeight)+qm_DiffRotateY
                }
            ...
            Item
            {
                ...
                Rectangle
                {
                    ...
                    TextInput
                    {
                        ...
                        validator: RegExpValidator { regExp: /[+0-9A-Za-z-@(),.*/\\;:"=_\s\[\]#$%^&!<>|\x20AC'?{}~`\x00A7]+/ }
                        passwordCharacter: "*"
