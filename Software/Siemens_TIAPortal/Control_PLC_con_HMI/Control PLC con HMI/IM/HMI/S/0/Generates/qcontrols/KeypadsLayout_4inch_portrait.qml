...
Rectangle
{
  ...
  function changeSpecialCharacters(startindex)
  {
      ...
      for(var row5index = 0; row5index < row5.children.length; row5index++)
      {
          row5.children[row5index].value = Keyboard.numbers[(2*Keyboard.rowLength) + startindex + row5index];
      }

      for(var row6index = 0; row6index < row6.children.length -1; row6index++)
      {
          row6.children[row6index + 1].value = Keyboard.numbers[(3*Keyboard.rowLength -1)+startindex+row6index];
      }

      row6.children[3].text = ( row6.children[0].text === specialkeysymbol1) ? Keyboard.numbers[26] : Keyboard.numbers[43];
  }
